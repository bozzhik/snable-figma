          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2.5">
              {report.snabled.url && getFavicon(report.snabled.url) && (
                <div className="grid p-0.5 bg-neutral-100 rounded-md place-items-center">
            ...
            <Text className="font-mono text-xs text-neutral-500 dark:text-neutral-400">{report.snabled.url.length > 40 ? report.snabled.url.substring(0, 40) + '..' : report.snabled.url}</Text>
          </div>

          <a href={report.snabled.url} target="_blank" className="inline-flex items-center p-2 transition-all duration-200 rounded-lg bg-neutral-100 dark:bg-neutral-700 hover:bg-neutral-200 dark:hover:bg-neutral-600 text-neutral-600 dark:text-neutral-300">
            <LinkIcon size={16} />
          </a>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(32px,1fr))] gap-1.5">
              {report.colors.slice(0, 7).map((color, index) => (
                <div key={index} className="w-full h-8 border-2 border-white rounded shadow-sm dark:border-neutral-600" style={{backgroundColor: color.value}} title={color.value} />
              ))}
              {report.colors.length > 7 && <div className="flex items-center justify-center w-full h-8 text-xs font-medium rounded bg-neutral-200 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400">+{report.colors.length - 7}</div>}
            </div>
          </div>
        )}
